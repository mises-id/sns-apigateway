package v1

import (
	"github.com/labstack/echo"
	"github.com/mises-id/sns-apigateway/app/apis/rest"
	"github.com/mises-id/sns-apigateway/lib/codes"

	pb "github.com/mises-id/sns-socialsvc/proto"
)

type BlacklistParams struct {
	UID uint64 `json:"uid"`
}

type ListBlacklistParams struct {
	rest.PageQuickParams
}

func ListBlacklist(c echo.Context) error {
	currentUID, ok := c.Get("CurrentUID").(uint64)
	if !ok {
		return codes.ErrUnauthorized
	}
	params := &ListBlacklistParams{}
	if err := c.Bind(params); err != nil {
		return codes.ErrInvalidArgument.Newf("invalid query params")
	}
	grpcsvc, ctx, err := rest.GrpcSocialService()
	if err != nil {
		return err
	}
	svcresp, err := grpcsvc.ListBlacklist(ctx, &pb.ListBlacklistRequest{Uid: currentUID, Paginator: &pb.PageQuick{
		NextId: params.PageQuickParams.NextID,
		Limit:  uint64(params.PageQuickParams.Limit),
	}})
	if err != nil {
		return err
	}
	return rest.BuildSuccessRespWithPagination(c, BuildBlacklistRespSlice(svcresp.Blacklists), svcresp.Paginator)
}

func CreateBlacklist(c echo.Context) error {
	currentUID, ok := c.Get("CurrentUID").(uint64)
	if !ok {
		return codes.ErrUnauthorized
	}
	params := &BlacklistParams{}
	if err := c.Bind(params); err != nil {
		return codes.ErrInvalidArgument.Newf("invalid query params")
	}
	grpcsvc, ctx, err := rest.GrpcSocialService()
	if err != nil {
		return err
	}
	_, err = grpcsvc.CreateBlacklist(ctx, &pb.CreateBlacklistRequest{Uid: currentUID, TargetUid: params.UID})
	if err != nil {
		return err
	}
	return rest.BuildSuccessResp(c, nil)
}

func DeleteBlacklist(c echo.Context) error {
	currentUID, ok := c.Get("CurrentUID").(uint64)
	if !ok {
		return codes.ErrUnauthorized
	}
	params := &BlacklistParams{}
	if err := c.Bind(params); err != nil {
		return codes.ErrInvalidArgument.Newf("invalid query params")
	}
	grpcsvc, ctx, err := rest.GrpcSocialService()
	if err != nil {
		return err
	}
	_, err = grpcsvc.DeleteBlacklist(ctx, &pb.DeleteBlacklistRequest{Uid: currentUID, TargetUid: params.UID})
	if err != nil {
		return err
	}
	return rest.BuildSuccessResp(c, nil)
}

func BuildBlacklistRespSlice(in []*pb.Blacklist) []*pb.Blacklist {
	if in == nil {
		return []*pb.Blacklist{}
	} else {
		return in
	}
}
